      ...
      <div className="container mx-auto px-6 w-full max-w-5xl">
        ...
        >
          ...
          <a
            ...
            aria-label={t('hero.subscribeAriaLabel')}
          ...
          >
            {t('hero.subscribe')} <ExternalLink className="w-5 h-5 ml-2" />
          </a>
        </motion.div>
        ...
        <div className="w-full">
          ...
          {loadingArticles ? (
            ...
            <div className="flex flex-col items-center py-20 text-muted-foreground">
              ...
              <p className="font-medium mb-2">{t('error.message')}</p>
              <a
                href="https://falacomigo.substack.com"
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t('error.fallbackAriaLabel')}
                className="text-primary font-semibold hover:underline inline-flex items-center"
                onClick={() => trackEvent('newsletter_click', { location: 'newsletter_error_fallback' })}
              >
                {t('error.fallback')} <ArrowRight className="w-4 h-4 ml-1" />
              </a>
            </div>
          ) : (
            <div className="divide-y divide-border mb-16">
              {articles.map((article) => (
                <motion.a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={t('articles.readAriaLabel', { title: article.title })}
                  ...
                  <div className="flex flex-col justify-center min-w-0 flex-grow">
                    <div className="flex items-center gap-3 text-xs font-medium text-muted-foreground mb-2">
                      <span className="inline-flex items-center gap-1">
                        ...
                        {new Date(article.publishedAt).toLocaleDateString(dateLocale, { month: 'long', day: 'numeric', year: 'numeric' })}
                      </span>
                      {article.readTime > 0 && (
                        <span className="inline-flex items-center gap-1">
                          <Clock className="w-3.5 h-3.5" />
                          {t('articles.minRead', { minutes: article.readTime })}
                        </span>
                      )}
                    </div>
        ...
        >
          <h2 className="text-xl font-bold text-foreground mb-3">{t('notes.title')}</h2>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            aria-label={t('notes.browseAriaLabel')}
          ...
          >
            {t('notes.browse')} <ExternalLink className="w-4 h-4 ml-2" />
          </a>
        </motion.div>
        ...
        >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            aria-label={t('cta.subscribeAriaLabel')}
            className="inline-flex items-center justify-center px-8 py-4 bg-primary text-primary-foreground text-lg font-bold rounded-xl hover:bg-primary/90 transition-colors"
            onClick={() => trackEvent('newsletter_click', { location: 'newsletter_bottom_cta' })}
          >
            {t('cta.subscribe')} <ExternalLink className="w-5 h-5 ml-2" />
          </a>
        </motion.div>
